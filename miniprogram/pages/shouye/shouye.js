...
Page({
  ...
  drawPie(index) {
    ...
    for (var i = 0; i < dataList.length; i++) {
      ...
      if (i < dataList.length - 1) {
        if (index == i) {
          ctxPie.arc(
            ...
            pieY,
            pieRadius + 5,
            (start / totalValue) * 2 * Math.PI,
            ((start + dataList[i].value) / totalValue) * 2 * Math.PI
          );
        } else {
          ctxPie.arc(
            pieX,
      ...
      ctxPie.fillText(
        parseInt((dataList[i].value * 100) / totalValue) + "%" + "",
        startX + 100,
        startY + 15
      );
    }
  ...
  touchStart(e) {
    ...
    var y = e.touches[0].y;
    if (
      Math.pow(x - pieInfo.centerX, 2) + Math.pow(y - pieInfo.centerY, 2) >
      Math.pow(pieInfo.pieRadius, 2)
    ) {
      console.log("在圆外，不执行");
      return;
    }
    ...
    var angle =
      Math.atan((y - pieInfo.centerY) / (x - pieInfo.centerX)) /
      (Math.PI / 180);
    //判断角度值
